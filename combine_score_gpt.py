...
for client_data in scenario:
    ...
    for data in client_data['datasets']:
        ...
        if data['type'] == 'open-ended':
            ...
            total_score = 0
            if is_client:
                with jsonlines.open(f'./eval_results_gpt/{mode}/{Method}/client{id}_round{num_rounds}_{data_name}.jsonl') as read_file:
                    for line in read_file.iter():
                        total_score += line['score']
                        count+=1
            else:
                with jsonlines.open(f'./eval_results_gpt/{mode}/{Method}/server_round{num_rounds}_{data_name}.jsonl') as read_file:
                    for line in read_file.iter():
                        total_score += line['score']
                        count+=1
