...
def fedavg_layerwise_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict):
    training_args.max_seq_length = training_args.model_max_length
    training_args.packing=False
    trainer = LLaVATrainerFEDAVG_Layerwise(model=model,
        tokenizer=tokenizer,
        args=training_args,
...
class LLaVATrainerFEDAVG_Layerwise(LLaVATrainerFEDAVG):
    def __init__(self, client_id, curr_round, test_datalist, processor, data_args, task_vector=None, fisher_old=None, fisher_freq=5,model2=None, **kwargs):
        super(LLaVATrainerFEDAVG_Layerwise, self).__init__(client_id=client_id,curr_round=curr_round,test_datalist=test_datalist,processor=processor,data_args=data_args,
                                                           task_vector=task_vector,fisher_old=fisher_old,fisher_freq=fisher_freq,model2=model2,
                                                           **kwargs)

    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        inputs['output_hidden_states'] = True
        loss, outputs = super(LLaVATrainerFEDAVG_Layerwise, self).compute_loss(model, inputs, return_outputs=True, num_items_in_batch=num_items_in_batch)
        if 'fedMultipqfullfreeze' in self.args.mode:
