...
def fedours_pqgrad_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict):
    ...
    trainer = LLaVATrainerOURS_PQGRAD(model=model,
        tokenizer=tokenizer,
        args=training_args,
...
class LLaVATrainerOURS_PQGRAD(LLaVATrainerFEDAVG):
    def __init__(self, task_id, ema_ratio=0.996, task_vector=None, fisher_old=None, fisher_freq=5, **kwargs):
        super(LLaVATrainerOURS_PQGRAD, self).__init__(**kwargs)
        ...
        self.task_vector= None #task_vector.cuda() if task_vector is not None and 'tv' not in self.args.mode else None
        self.fisher_old = [f.cuda() for f in self.fisher_old] if fisher_old is not None else None #{k:p.cuda() for k, p in fisher_old.items()} if fisher_old is not None else None
        # self.fisher_cur = 0
        # self.fisher_cnt = 0
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        #     #     model.module.set_state('gate')
            ...
            #     model.module.set_state('lora2')
        loss, outputs = super(LLaVATrainerOURS_PQGRAD, self).compute_loss(model, inputs, return_outputs=True, num_items_in_batch=num_items_in_batch)
    ...
    def _inner_training_loop(
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    if do_sync_step:
                        ...
                        if 'tv' not in args.mode and 'excludemean' not in args.mode and ((step-args.gradient_accumulation_steps+1)) % self.fisher_freq == 0:
                            ...
                            if 'pqfisher' in args.mode:
                                if self.fisher_old is None:
                                    self.fisher_old = [(g**2).detach() for g in grads]
                                else:
                                    self.fisher_old = [(self.fisher_ema_ratio)*g_ema + (1-self.fisher_ema_ratio)*(g**2).detach() for g_ema, g in zip(self.fisher_old, grads)]
                            elif 'pqgrad' in args.mode:
                                if self.fisher_old is None:
                                    self.fisher_old = [(g).detach() for g in grads]
                                else:
                                    self.fisher_old = [(self.fisher_ema_ratio)*g_ema + (1-self.fisher_ema_ratio)*(g).detach() for g_ema, g in zip(self.fisher_old, grads)]
                            # for p in self.model2.base_model.language_model.model.layers[-1].mlp.down_proj.base_layer.parameters():
        ...
        if 'tv' not in args.mode and 'excludemean' not in args.mode:
            ...
            self.task_vector = self.fisher_old = [g.detach().cpu() for g in self.fisher_old]
