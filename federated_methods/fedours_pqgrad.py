...
class LLaVATrainerOURS_PQGRAD(LLaVATrainerFEDAVG):
    def __init__(self, task_id, ema_ratio=0.996, task_vector=None, fisher_old=None, fisher_freq=5, **kwargs):
        ...
        self.fisher_old = [f.cuda() for f in fisher_old] if fisher_old is not None else None #{k:p.cuda() for k, p in fisher_old.items()} if fisher_old is not None else None
        # self.fisher_cur = 0
        # self.fisher_cnt = 0
