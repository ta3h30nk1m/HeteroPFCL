...
def fedours_smooth_create_trainer(model, tokenizer, training_args, data_module, extra_state_dict_dict):
    ...
    trainer = LLaVATrainerOURS(model=model,
        ...
        new_global_state_dict=extra_state_dict_dict['new_global_state_dict'] if 'new_global_state_dict' in extra_state_dict_dict else None,
        **data_module,
        )
    return trainer


def fedours_smooth_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict=None):
...
def compute_ema_ratio(max_iter, epsilon=1e-2):
    return math.exp(math.log(epsilon) / max_iter)

class LLaVATrainerOURS(LLaVATrainerFEDAVG):
    def __init__(self, task_id, ema_ratio=0.996, task_vector=None, fisher_old=None, fisher_freq=5, model2=None,new_global_state_dict=None,**kwargs):
        ...
        self.new_global_state_dict = new_global_state_dict
        if self.new_global_state_dict is not None:
            self.new_global_state_dict = {k:v.cuda() for k,v in self.new_global_state_dict.items()}
            if self.args.mode == 'fedours_moe_smooth_linearC':
                self.initial_global_weights = {
                    k: v.clone().detach().cuda() for k, v in self.model.named_parameters() if 'lora1' in k
                }
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            self.ema_ratio = compute_ema_ratio(total_updates-1, epsilon=1e-3)
            for local_update_i in range(total_updates):
                update_step += 1
                ...
                for i, inputs in enumerate(batch_samples):
                    ...
                    if do_sync_step:
                        ...
                        if self.new_global_state_dict is not None:
                            with torch.no_grad():
                                cur_global_weights = {k: t for k, t in self.model.named_parameters() if 'lora1' in k}
                                if args.mode == 'fedours_moe_smooth_linear':
                                    alpha = min( local_update_i / (total_updates-1), 1.0)
                                    for k, new_param in self.new_global_state_dict.items():
                                        if k in cur_global_weights:
                                            cur_global_weights[k].copy_((1 - alpha) * cur_global_weights[k] + alpha * new_param)
                                elif args.mode == 'fedours_moe_smooth_linearC':
                                    alpha = min( local_update_i / (total_updates-1), 1.0)
                                    for k, new_param in self.new_global_state_dict.items():
                                        if k in cur_global_weights:
