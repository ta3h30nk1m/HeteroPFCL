...
def fedMulti2pq_back_HomoAgg_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
    # first load loca model and then load global model
    layer_index = extra_state_dict_dict['LAYER_INDEX']
    with torch.no_grad():
        if 'zero3' in training_args.deepspeed:
...
def feddualMulti05pq_include_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
    # first load loca model and then load global model
    layer_index = extra_state_dict_dict['LAYER_INDEX']
    with torch.no_grad():
        if 'zero3' in training_args.deepspeed:
...
def feddualMultipqfull_homoAgg_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
    # first load loca model and then load global model
    layer_index = extra_state_dict_dict['LAYER_INDEX']
    with torch.no_grad():
        if 'zero3' in training_args.deepspeed:
...
def feddualMultipq_include_homoAgg_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
    # first load loca model and then load global model
    layer_index = extra_state_dict_dict['LAYER_INDEX']
    with torch.no_grad():
        if 'zero3' in training_args.deepspeed:
...
def feddualMulti05pq_include_homoAgg_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
    # first load loca model and then load global model
    layer_index = extra_state_dict_dict['LAYER_INDEX']
    with torch.no_grad():
        if 'zero3' in training_args.deepspeed:
...
def feddualMulti2pqfull_homoAgg_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
    layer_index = extra_state_dict_dict['LAYER_INDEX']
    # first load loca model and then load global model
    with torch.no_grad():
        if 'zero3' in training_args.deepspeed:
...
def feddualMultipq_homoAgg_Normalize_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
    # first load loca model and then load global model
    layer_index = extra_state_dict_dict['LAYER_INDEX']
    with torch.no_grad():
        if 'zero3' in training_args.deepspeed:
        ...
        if extra_state_dict_dict['curr_round'] > 0 and 'task_similarity' in extra_state_dict_dict:
            ...
            for name in global_state_dict.keys():
                ...
                current_client_norm = torch.norm(local_state_dict_list[client_id][target_key])
                splited = target_key.split('.')
                if int(splited[layer_index]) in cur_layer_num:
                    if 'lora2_P' not in target_key and 'lora2_Q' not in target_key:
                        continue
                    ...
                    for id in range(training_args.num_clients):
                        if id == client_id:
                            ...
                            if cur_layer_num[-1] != target_layers[-1]: # if different size
                                ...
                                new_target_key = target_key
                            new_param += (weights[id]/ sim_sum)*(current_client_norm / torch.norm(local_state_dict_list[id][new_target_key]))*local_state_dict_list[id][new_target_key] 
                else:
                    for id in homo_client_ids:
                        if id == client_id:
                            continue
                        new_param +=(homo_weights[id]/ homo_sim_sum)*(current_client_norm/torch.norm(local_state_dict_list[id][target_key]))*local_state_dict_list[id][target_key] 
                if isinstance(new_param, int):
                    continue
                new_global_state_dict[name] = new_param
            # if (training_args.local_rank == 0 or training_args.local_rank == -1):
...
def feddualMulti05pq_homoAgg_Normalize_load_state_dict(model, global_state_dict, local_state_dict_list, client_id, training_args, extra_state_dict_dict):
    # first load loca model and then load global model
    layer_index = extra_state_dict_dict['LAYER_INDEX']
    with torch.no_grad():
        if 'zero3' in training_args.deepspeed:
