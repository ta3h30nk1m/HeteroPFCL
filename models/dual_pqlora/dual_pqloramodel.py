...
from .dual_pqloralayer import Conv2d, PQLoraLayer, dispatch_default
...
class Dual_PQLoraModel(BaseTuner):
    """
    ...
    **Attributes**:
    ...
    def _create_and_replace(
        ...
        if isinstance(target, PQLoraLayer) and not isinstance(target, AdaLoraLayer):
            target.update_layer(
                adapter_name,
                r,
                lora_alpha=alpha,
        ...
        for active_adapter in self.active_adapters:
            ...
            if bias == "all":
                ...
                for m in model.modules():
                    if isinstance(m, PQLoraLayer) and hasattr(m, "bias") and m.bias is not None:
    ...
    def set_adapter(self, adapter_name: str | list[str]) -> None:
        ...
        """
        for module in self.model.modules():
            if isinstance(module, PQLoraLayer):
                if module.merged:
    ...
    def _enable_peft_forward_hooks(self, *args, **kwargs):
        ...
        for layer in self.modules():
            if isinstance(layer, PQLoraLayer):
        ...
        for module in self.modules():
            if isinstance(module, PQLoraLayer) or isinstance(module, ModulesToSaveWrapper):
    ...
    def add_weighted_adapter(
        ...
        for key in key_list:
            ...
            if isinstance(target, PQLoraLayer):
