...
class LoraInLora(nn.Module):
    def __init__(
        ...
        self.bias = bias
        self.has_non_linearity = has_non_linearity
        
        self.non_linearity = nn.SiLU(inplace=True)
        
        self.layer1 = nn.Linear(self.in_features, self.hidden_features, bias=False)
        self.layer2 = nn.Linear(self.hidden_features, self.out_features, bias=bias)
    ...
    def init_random(self):
        ...
        # if self.bias:
        #     nn.init.zeros_(self.layer2.bias, a=math.sqrt(5))

    def forward(self, x: torch.Tensor):
        if self.has_non_linearity:
            out = self.layer2(self.non_linearity(self.layer1(x)))
        else:
            out = self.layer2(self.layer1(x))
        return out


class PQLoraLILFullFreezeLayer(BaseTunerLayer):
    # All names of layers that may contain (trainable) adapter weights
    adapter_layer_names = ("lora1_A", "lora1_B", "lora1_embedding_A", "lora1_embedding_B", "lora1_P",
                           "lora2_A", "lora2_B", "lora2_embedding_A", "lora2_embedding_B", "lora2_P")
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.lora1_P = nn.ModuleDict({})
        self.lora2_P = nn.ModuleDict({})
        # For Embedding layer
    ...
    def update_layer(
        ...
        self.lora1_P[adapter_name] = LoraInLora(r,r,r//8)  # Initialized to 1
        self.lora2_P[adapter_name] = copy.deepcopy(self.lora1_P[adapter_name])  # Initialized to 1
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora1_A.keys():
            ...
            self.lora1_P[adapter_name].init_random()
            with torch.no_grad():
                self.lora2_P[adapter_name].layer1.weight.copy_(self.lora1_P[adapter_name].layer1.weight.data)
                self.lora2_P[adapter_name].layer2.weight.copy_(self.lora1_P[adapter_name].layer2.weight.data)
                if self.lora2_P[adapter_name].bias:
                    self.lora2_P[adapter_name].layer2.bias.copy_(self.lora1_P[adapter_name].layer2.bias.data)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            if self.active_state == 'lora1':
                lora_output = lora1_B(lora1_P(lora1_A(dropout(sub_batch)))) * scaling
            elif self.active_state =='lora2':
                lora_output = lora2_B(lora2_P(lora2_A(dropout(sub_batch)))) * scaling
            elif self.active_state =='gate':
                lora_output = (lora1_B(lora1_P(lora1_A(dropout(sub_batch)))) + lora2_B(lora2_P(lora2_A(dropout(sub_batch))))) * scaling / 2
...
class Linear(nn.Module, PQLoraLILFullFreezeLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        PQLoraLILFullFreezeLayer.__init__(self, base_layer, **kwargs)
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    if self.use_pq:
                        if self.active_state == 'lora1':
                            result = result + lora1_B(lora1_P(lora1_A(dropout(x)))) * scaling
                        elif self.active_state == 'lora2':
...
class Embedding(nn.Module, PQLoraLILFullFreezeLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        PQLoraLILFullFreezeLayer.__init__(self, base_layer)
...
class _ConvNd(nn.Module, PQLoraLILFullFreezeLayer):
