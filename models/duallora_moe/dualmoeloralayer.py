...
class DualMOELoraLayer(BaseTunerLayer):
    ...
    def update_layer(
        ...
        self.lora_w_gate[adapter_name] = nn.Parameter(torch.zeros(self.in_features,2), requires_grad=True)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            if self.active_state == 'lora1':
                ...
                lora_output = weights[:,0].unsqueeze(-1).unsqueeze(-1)  * lora1_B(lora1_A(dropout(sub_batch))) * scaling + weights[:,1].unsqueeze(-1).unsqueeze(-1)  * lora2_B(lora2_A(dropout(sub_batch))) * scaling
...
class Linear(nn.Module, DualMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        DualMOELoraLayer.__init__(self, base_layer, **kwargs)
            ...
            for active_adapter in self.active_adapters:
                ...
                if self.active_state == 'lora1':
                    ...
                    if not self.use_dora[active_adapter]:
                        ...
                        result = result + weights[:,0].unsqueeze(-1).unsqueeze(-1) * lora1_B(lora1_A(dropout(x))) * scaling + weights[:,1].unsqueeze(-1).unsqueeze(-1)  * lora2_B(lora2_A(dropout(x))) * scaling
...
def moe_gating_weight(x, w_gate, w_noise, train=True, noise_epsilon=1e-5):
    ...
    if train:
        ...
        noise_stddev = ((nn.Softplus()(raw_noise_stddev) + noise_epsilon))
...
class Embedding(nn.Module, DualMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        DualMOELoraLayer.__init__(self, base_layer)
...
class _ConvNd(nn.Module, DualMOELoraLayer):
