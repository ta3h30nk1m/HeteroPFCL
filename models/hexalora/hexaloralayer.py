...
class HexaLoraLayer(BaseTunerLayer):
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            if self.active_state == 'lora1':
                ...
                lora_output = (lora1_B(lora1_A(dropout(sub_batch))) + lora2_B(lora2_A(dropout(sub_batch))) + lora3_B(lora3_A(dropout(sub_batch))) + lora4_B(lora4_A(dropout(sub_batch)))+ lora5_B(lora5_A(dropout(sub_batch)))+ lora6_B(lora6_A(dropout(sub_batch)))) * scaling / 6
...
class Linear(nn.Module, HexaLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        HexaLoraLayer.__init__(self, base_layer, **kwargs)
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if self.active_state == 'lora1':
                    ...
                    if not self.use_dora[active_adapter]:
                        result = result + (lora1_B(lora1_A(dropout(x))) + lora2_B(lora2_A(dropout(x))) + lora3_B(lora3_A(dropout(x))) + lora4_B(lora4_A(dropout(x)))+ lora5_B(lora5_A(dropout(x)))+ lora6_B(lora6_A(dropout(x)))) * scaling / 6
                    else:
                        if isinstance(dropout, nn.Identity) or not self.training:
                            base_result = result
                        else:
...
class Embedding(nn.Module, HexaLoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        HexaLoraLayer.__init__(self, base_layer)
...
class _ConvNd(nn.Module, HexaLoraLayer):
