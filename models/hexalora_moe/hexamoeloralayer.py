...
class HexaMOELoraLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora1_A", "lora1_B", "lora1_embedding_A", "lora1_embedding_B",
                           ...
                           "lora5_A", "lora5_B", "lora5_embedding_A", "lora5_embedding_B",
                           "lora6_A", "lora6_B", "lora6_embedding_A", "lora6_embedding_B",
                           "lora_w_gate", "lora_w_noise")
    
    # lora1: global
    # lora2: local
    # lora3: local ema 0.99
    # lora4: local ema 0.9
    # lora5: global ema 0.99
    # lora6: global ema 0.9
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.lora5_A = nn.ModuleDict({})
        self.lora5_B = nn.ModuleDict({})
        self.lora6_A = nn.ModuleDict({})
        self.lora6_B = nn.ModuleDict({})
        ...
        self.lora5_embedding_A = nn.ParameterDict({})
        self.lora5_embedding_B = nn.ParameterDict({})
        self.lora6_embedding_A = nn.ParameterDict({})
        self.lora6_embedding_B = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        self.lora5_A[adapter_name] = copy.deepcopy(self.lora1_A[adapter_name])#nn.Linear(self.in_features, r, bias=False)
        self.lora5_B[adapter_name] = copy.deepcopy(self.lora1_B[adapter_name])#nn.Linear(r, self.out_features, bias=False)
        
        self.lora6_A[adapter_name] = copy.deepcopy(self.lora1_A[adapter_name])#nn.Linear(self.in_features, r, bias=False)
        self.lora6_B[adapter_name] = copy.deepcopy(self.lora1_B[adapter_name])#nn.Linear(r, self.out_features, bias=False)
        ...
        self.lora_w_gate[adapter_name] = nn.Parameter(torch.zeros(self.in_features,6), requires_grad=True)
        self.lora_w_noise[adapter_name] = nn.Parameter(torch.zeros(self.in_features,6), requires_grad=True)
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora1_A.keys():
            if init_lora_weights is True:
                ...
                self.lora5_A[adapter_name].weight.data.copy_(init_A)
                self.lora6_A[adapter_name].weight.data.copy_(init_A)
            ...
            nn.init.zeros_(self.lora5_B[adapter_name].weight)
            nn.init.zeros_(self.lora6_B[adapter_name].weight)
            if self.lora_bias[adapter_name]:
                ...
                nn.init.zeros_(self.lora5_B[adapter_name].bias)
                nn.init.zeros_(self.lora6_B[adapter_name].bias)
        if adapter_name in self.lora1_embedding_A.keys():
            ...
            nn.init.zeros_(self.lora5_embedding_A[adapter_name])
            nn.init.zeros_(self.lora6_embedding_A[adapter_name])
            ...
            self.lora5_embedding_B[adapter_name].data.copy_(init_B)
            self.lora6_embedding_B[adapter_name].data.copy_(init_B)
            if self.lora_bias[adapter_name]:
                ...
                nn.init.zeros_(self.lora5_embedding_B[adapter_name].bias)
                nn.init.zeros_(self.lora6_embedding_B[adapter_name].bias)
    ...
    def olora_init(self, adapter_name):
        ...
        self.lora5_A[adapter_name].weight.data = Rr.contiguous()
        self.lora5_B[adapter_name].weight.data = Qr.contiguous()
        
        self.lora6_A[adapter_name].weight.data = Rr.contiguous()
        self.lora6_B[adapter_name].weight.data = Qr.contiguous()
    ...
    def pissa_init(self, adapter_name, init_lora_weights):
        ...
        self.lora5_A[adapter_name].weight.data = lora_A
        self.lora5_B[adapter_name].weight.data = lora_B
        self.lora6_A[adapter_name].weight.data = lora_A
        self.lora6_B[adapter_name].weight.data = lora_B
    ...
    def loftq_init(self, adapter_name):
        ...
        if adapter_name in self.lora1_embedding_A.keys():
            ...
            self.lora5_embedding_A[adapter_name].weight.data = lora_A
            self.lora5_embedding_B[adapter_name].weight.data = lora_B
            self.lora6_embedding_A[adapter_name].weight.data = lora_A
            self.lora6_embedding_B[adapter_name].weight.data = lora_B
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            lora5_A = self.lora5_A[active_adapter]
            lora5_B = self.lora5_B[active_adapter]
            lora6_A = self.lora6_A[active_adapter]
            lora6_B = self.lora6_B[active_adapter]
            ...
            if self.active_state == 'lora1':
            ...
            elif self.active_state =='lora5':
                lora_output = lora5_B(lora5_A(dropout(sub_batch))) * scaling
            elif self.active_state =='lora6':
                lora_output = lora6_B(lora6_A(dropout(sub_batch))) * scaling
                ...
                lora_output = weights[:,0].unsqueeze(-1).unsqueeze(-1)*lora1_B(lora1_A(dropout(sub_batch)))*scaling \
                            + weights[:,1].unsqueeze(-1).unsqueeze(-1)*lora2_B(lora2_A(dropout(sub_batch)))*scaling \
                            + weights[:,2].unsqueeze(-1).unsqueeze(-1)*lora3_B(lora3_A(dropout(sub_batch)))*scaling \
                            + weights[:,3].unsqueeze(-1).unsqueeze(-1)*lora4_B(lora4_A(dropout(sub_batch)))*scaling \
                            + weights[:,4].unsqueeze(-1).unsqueeze(-1)*lora5_B(lora5_A(dropout(sub_batch)))*scaling \
                            + weights[:,5].unsqueeze(-1).unsqueeze(-1)*lora6_B(lora6_A(dropout(sub_batch)))*scaling
            result[sub_batch_indices_list[i]] += lora_output.to(torch_result_dtype)

        return result

    def set_state(self, state):
        assert state in ['lora1', 'lora2','lora3','lora4', 'lora5','lora6', 'gate'], state
    ...
    def activate_all(self):
        ...
        for p in self.lora4_B.parameters():
            p.requires_grad = True
        for p in self.lora5_A.parameters():
            p.requires_grad = True
        for p in self.lora5_B.parameters():
            p.requires_grad = True
        for p in self.lora6_A.parameters():
            p.requires_grad = True
        for p in self.lora6_B.parameters():
            p.requires_grad = True
        ...
        for p in self.lora4_embedding_B.parameters():
            p.requires_grad = True
        for p in self.lora5_embedding_A.parameters():
            p.requires_grad = True
        for p in self.lora5_embedding_B.parameters():
            p.requires_grad = True
        for p in self.lora6_embedding_A.parameters():
            p.requires_grad = True
        for p in self.lora6_embedding_B.parameters():
            p.requires_grad = True
...
class Linear(nn.Module, HexaMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        HexaMOELoraLayer.__init__(self, base_layer, **kwargs)
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        weight5_A = self.lora5_A[adapter].weight
        weight5_B = self.lora5_B[adapter].weight
        weight6_A = self.lora6_A[adapter].weight
        weight6_B = self.lora6_B[adapter].weight
        if self.active_state == 'lora1':
            if cast_to_fp32:
            ...
            if cast_to_fp32:
                weight5_A = weight5_A.float()
                weight5_B = weight5_B.float()

            output_tensor = transpose(weight5_B @ weight5_A, self.fan_in_fan_out) * self.scaling[adapter]

            if cast_to_fp32:
                output_tensor = output_tensor.to(dtype=dtype)

                # cast back the weights
                self.lora5_A[adapter].weight.data = weight5_A.to(dtype)
                self.lora5_B[adapter].weight.data = weight5_B.to(dtype)
        elif self.active_state == 'lora6':
            if cast_to_fp32:
                weight6_A = weight6_A.float()
                weight6_B = weight6_B.float()

            output_tensor = transpose(weight6_B @ weight6_A, self.fan_in_fan_out) * self.scaling[adapter]

            if cast_to_fp32:
                output_tensor = output_tensor.to(dtype=dtype)

                # cast back the weights
                self.lora6_A[adapter].weight.data = weight6_A.to(dtype)
                self.lora6_B[adapter].weight.data = weight6_B.to(dtype)
        elif self.active_state == 'gate':
            if cast_to_fp32:
                weight1_A = weight1_A.float()
                weight1_B = weight1_B.float()
            ...
            output_tensor = (transpose(weight1_B @ weight1_A, self.fan_in_fan_out)+transpose(weight2_B @ weight2_A, self.fan_in_fan_out)+transpose(weight3_B @ weight3_A, self.fan_in_fan_out)+transpose(weight4_B @ weight4_A, self.fan_in_fan_out)+transpose(weight5_B @ weight5_A, self.fan_in_fan_out)+transpose(weight6_B @ weight6_A, self.fan_in_fan_out)) * self.scaling[adapter] / 6
        ...
        if self.disable_adapters:
            ...
            for active_adapter in self.active_adapters:
                ...
                if self.active_state == 'lora1':
                    ...
                    if not self.use_dora[active_adapter]:
                        result = result + lora5_B(lora5_A(dropout(x))) * scaling
                    else:
                        if isinstance(dropout, nn.Identity) or not self.training:
                            base_result = result
                        else:
                    ...
                    if not self.use_dora[active_adapter]:
                        result = result + lora6_B(lora6_A(dropout(x))) * scaling
                    else:
                        if isinstance(dropout, nn.Identity) or not self.training:
                            base_result = result
                        else:
                    ...
                    if not self.use_dora[active_adapter]:
                        weights = moe_gating_weight(x, self.lora_w_gate[active_adapter], self.lora_w_noise[active_adapter], train=self.training)
                        result = result + weights[:,0].unsqueeze(-1).unsqueeze(-1)*lora1_B(lora1_A(dropout(x))) * scaling \
                                        + weights[:,1].unsqueeze(-1).unsqueeze(-1)*lora2_B(lora2_A(dropout(x))) * scaling \
                                        + weights[:,2].unsqueeze(-1).unsqueeze(-1)*lora3_B(lora3_A(dropout(x))) * scaling \
                                        + weights[:,3].unsqueeze(-1).unsqueeze(-1)*lora4_B(lora4_A(dropout(x))) * scaling \
                                        + weights[:,4].unsqueeze(-1).unsqueeze(-1)*lora5_B(lora5_A(dropout(x))) * scaling \
                                        + weights[:,5].unsqueeze(-1).unsqueeze(-1)*lora6_B(lora6_A(dropout(x))) * scaling
                    else:
                        if isinstance(dropout, nn.Identity) or not self.training:
                            base_result = result
                        else:
...
class Embedding(nn.Module, HexaMOELoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        HexaMOELoraLayer.__init__(self, base_layer)
...
class _ConvNd(nn.Module, HexaMOELoraLayer):
