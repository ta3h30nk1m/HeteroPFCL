...
from models.quadralora_moe.quadramoeloralayer import moe_gating_weight, cv_squared

class QuadraPQMOELoraFullFreezeLayer(BaseTunerLayer):
    ...
    adapter_layer_names = ("lora1_A", "lora1_B", "lora1_embedding_A", "lora1_embedding_B", "lora1_P", "lora1_Q",
                           ...
                           "lora3_A", "lora3_B", "lora3_embedding_A", "lora3_embedding_B", "lora3_P", "lora3_Q",
                           "lora4_A", "lora4_B", "lora4_embedding_A", "lora4_embedding_B", "lora4_P", "lora4_Q",
    ...
    def __init__(self, base_layer: nn.Module, ephemeral_gpu_offload: bool = False, **kwargs) -> None:
        ...
        self.lora3_P = nn.ParameterDict({})
        self.lora3_Q = nn.ParameterDict({})
        self.lora4_P = nn.ParameterDict({})
        self.lora4_Q = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        self.lora3_A[adapter_name] = copy.deepcopy(self.lora1_A[adapter_name])
        self.lora3_B[adapter_name] = copy.deepcopy(self.lora1_B[adapter_name])
        
        self.lora4_A[adapter_name] = copy.deepcopy(self.lora1_A[adapter_name])
        self.lora4_B[adapter_name] = copy.deepcopy(self.lora1_B[adapter_name])
        ...
        self.lora3_P[adapter_name] = nn.Parameter(torch.eye((r)))  # Initialized to 1
        self.lora3_Q[adapter_name] = nn.Parameter(torch.zeros((1,r))) # Initialized to 0
        self.lora4_P[adapter_name] = nn.Parameter(torch.eye((r)))  # Initialized to 1
        self.lora4_Q[adapter_name] = nn.Parameter(torch.zeros((1,r))) # Initialized to 0
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora1_A.keys():
            ...
            self.lora3_Q[adapter_name].data.copy_(init_Q)
            self.lora4_Q[adapter_name].data.copy_(init_Q)
    ...
    def _mixed_batch_forward(
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            lora3_P = self.lora3_P[active_adapter]
            lora3_Q = self.lora3_Q[active_adapter]
            lora4_P = self.lora4_P[active_adapter]
            lora4_Q = self.lora4_Q[active_adapter]
            ...
            if self.use_pq:
                if self.active_state == 'lora1':
                    ...
                    lora_output = lora3_B(lora3_A(dropout(sub_batch))@lora3_P + lora3_Q) * scaling
                elif self.active_state =='lora4':
                    lora_output = lora4_B(lora4_A(dropout(sub_batch))@lora4_P + lora4_Q) * scaling
                    ...
                    lora_output = (weights[:,0].unsqueeze(-1).unsqueeze(-1) *lora1_B(lora1_A(dropout(sub_batch))@lora1_P + lora1_Q) 
                                   +weights[:,1].unsqueeze(-1).unsqueeze(-1) *lora2_B(lora2_A(dropout(sub_batch))@lora2_P + lora2_Q)
                                   +weights[:,2].unsqueeze(-1).unsqueeze(-1) *lora3_B(lora3_A(dropout(sub_batch))@lora3_P + lora3_Q)
                                   +weights[:,3].unsqueeze(-1).unsqueeze(-1) *lora4_B(lora4_A(dropout(sub_batch))@lora4_P + lora4_Q)) * scaling
            else:
                if self.active_state == 'lora1':
                    ...
                    weights = moe_gating_weight(x, self.lora_w_gate[active_adapter], self.lora_w_noise[active_adapter], train=self.training)
                    lora_output = (weights[:,0].unsqueeze(-1).unsqueeze(-1) *lora1_B(lora1_A(dropout(sub_batch))) + weights[:,1].unsqueeze(-1).unsqueeze(-1) *lora2_B(lora2_A(dropout(sub_batch)))+weights[:,2].unsqueeze(-1).unsqueeze(-1) *lora3_B(lora3_A(dropout(sub_batch)))+weights[:,3].unsqueeze(-1).unsqueeze(-1) *lora4_B(lora4_A(dropout(sub_batch)))) * scaling
    ...
    def activate_all(self):
        ...
        for p in self.lora2_Q.parameters():
            p.requires_grad = True
        for p in self.lora3_P.parameters():
            p.requires_grad = True
        for p in self.lora3_Q.parameters():
            p.requires_grad = True
        for p in self.lora4_P.parameters():
            p.requires_grad = True
        for p in self.lora4_Q.parameters():
            p.requires_grad = True
...
class Linear(nn.Module, QuadraPQMOELoraFullFreezeLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        QuadraPQMOELoraFullFreezeLayer.__init__(self, base_layer, **kwargs)
                ...
                if not self.use_dora[active_adapter]:
                    if self.use_pq:
                        if self.active_state == 'lora1':
                            ...
                            result = result + lora3_B(lora3_A(dropout(x)) @ lora3_P + lora3_Q) * scaling
                        elif self.active_state == 'lora4':
                            result = result + lora4_B(lora4_A(dropout(x)) @ lora4_P + lora4_Q) * scaling
                        elif self.active_state == 'gate':
                            weights = moe_gating_weight(x, self.lora_w_gate[active_adapter], self.lora_w_noise[active_adapter], train=self.training)
                            result = result + (weights[:,0].unsqueeze(-1).unsqueeze(-1) *lora1_B(lora1_A(dropout(x)) @ lora1_P + lora1_Q) 
                                               + weights[:,1].unsqueeze(-1).unsqueeze(-1) *lora2_B(lora2_A(dropout(x)) @ lora2_P + lora2_Q)
                                               + weights[:,2].unsqueeze(-1).unsqueeze(-1) *lora3_B(lora3_A(dropout(x)) @ lora3_P + lora3_Q)
                                               + weights[:,3].unsqueeze(-1).unsqueeze(-1) *lora4_B(lora4_A(dropout(x)) @ lora4_P + lora4_Q)) * scaling
                    else:
                        if self.active_state == 'lora1':
                            ...
                            weights = moe_gating_weight(x, self.lora_w_gate[active_adapter], self.lora_w_noise[active_adapter], train=self.training)
                            result = result + (weights[:,0].unsqueeze(-1).unsqueeze(-1) *lora1_B(lora1_A(dropout(x))) + weights[:,1].unsqueeze(-1).unsqueeze(-1) *lora2_B(lora2_A(dropout(x))) + weights[:,2].unsqueeze(-1).unsqueeze(-1) *lora3_B(lora3_A(dropout(x))) + weights[:,3].unsqueeze(-1).unsqueeze(-1) *lora4_B(lora4_A(dropout(x)))) * scaling
                else:
...
class Embedding(nn.Module, QuadraPQMOELoraFullFreezeLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        super().__init__()
        QuadraPQMOELoraFullFreezeLayer.__init__(self, base_layer)
...
class _ConvNd(nn.Module, QuadraPQMOELoraFullFreezeLayer):
