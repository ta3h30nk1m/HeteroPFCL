...
json_train_datalist = np.random.choice(json_train_datalist[0], size=1333, replace=False).tolist() + np.random.choice(json_train_datalist[1], size=1333, replace=False).tolist() + np.random.choice(json_train_datalist[2], size=1333, replace=False).tolist()
json_test_datalist = np.random.choice(json_test_datalist[0], size=334, replace=False).tolist() + np.random.choice(json_test_datalist[1], size=334, replace=False).tolist() + np.random.choice(json_test_datalist[2], size=334, replace=False).tolist()

print(len(json_train_datalist))
print(len(json_test_datalist))
with open(f'{train_folder}/dataset-30.json', 'w') as json_file:
    json.dump(json_train_datalist, json_file, indent=4)

with open(f'{test_folder}/dataset-30.json', 'w') as json_file:
