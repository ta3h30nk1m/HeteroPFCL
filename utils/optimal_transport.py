...
def _get_neuron_importance_histogram(
    ...
    layer = layer_weight.cpu().float().numpy()
    if importance == "l1":
...
def get_wassersteinized_layers_modularized(
    ...
    for idx, key in enumerate(models[0].keys()):
        print(idx, key)
        # if not params0.requires_grad:
        #     continue
        params0 = models[0][key]
        params1 = models[1][key]
        if idx % 2 == 0:
            T_var = None
        ...
        if idx % 2 == 0:
            ...
            aligned_wt = torch.matmul(params0.data, T_var.type_as(params0_data))
        ...
        if past_correction:
            ...
            t_fc0_model = torch.matmul(
                T_var.t().type_as(params0_data), aligned_wt.contiguous().view(aligned_wt.shape[0], -1)
            )
        else:
            t_fc0_model = torch.matmul(
                T_var.t().type_as(params0_data),
                params0_data.view(params0_data.shape[0], -1),
            )

        avg_aligned_layers[key] = t_fc0_model
    return avg_aligned_layers, T_vars