...
def get_VLMmodel(model_args, training_args, bnb_model_from_pretrained_args, data_args):
    ...
    if training_args.lora_enable:
        ...
        if training_args.mode in ['fedours', 'fedours_tv', 'fedours_only_B_train', 'fedours_tv_only_B_train', 'fedours_excludemean','fedours_self',
                                  ...
                                  'perada','fedsim','fedsim_hetero','ditto','feddpa']:
            ...
            lora_config.peft_type = 'PQLORA'
                                    ...
                                    'fedMulti2pqfullfreezeA','fedMultipqfullfreezeA', 'fedMultipqfullfreezeA_sft','fedMultipqfullfreezeB_sft','fedMultipqfull_sft','fedMultipqfull_sft2',
            ...
            lora_config.peft_type = 'DUALPQFreezeALORA'
                                    ...
                                    'feddualMultipqfullfreeze', 'feddualMultipqfullfreeze_tv', 'feddualMultipqfullfreeze_excludemean', 'feddualMultipqfullfreeze_pqgrad','feddualMultipqfullfreeze_pqfisher','feddualMultipqfullfreeze_pqgrad_homoAgg',
        ...
        elif training_args.mode in ['feddualMultipfullfreeze', 'feddualMulti2pfullfreeze_back']:
        ...
        elif training_args.mode in ['feddualMultipfullfreeze_homoAgg_moe', 'feddualMulti05pfullfreeze_homoAgg_moe']:
            from models.dual_plora_freeze_full_moe.dual_ploramodel_freeze_full_moe import Dual_PMOELorafreezeModel
            from peft.peft_model import PEFT_TYPE_TO_MODEL_MAPPING
            ...
            lora_config.peft_type = 'DUALPMOEFullFreezeLORA'
                                    ...
                                    'feddualMultipqfullfreeze_excludemean_homoAgg_moe','feddualMultipqfullfreeze_pqgrad_homoAgg_moe',
                                    ]:
        ...
        elif training_args.mode in ['feddualMultipqfullfreezeA', 'feddualMultipqfullfreezeA_tv', 'feddualMultipqfullfreezeA_excludemean','feddualpqfullfreezeA','feddualMultipqfullfreezeA2','feddualMulti2pqfullfreezeA_back2']:
        ...
        elif training_args.mode in ['feddualMultipqfull','feddualMultipqfull2','feddualMulti2pqfull_back2']:
        ...
        elif training_args.mode in ['feddualMultipqfullfreezeA_homoAgg_moe','feddualMultipqfullfreezeA_homoAgg_moe2','feddualMulti2pqfullfreezeA_back_homoAgg_moe2',]:
            ...
            lora_config.peft_type = 'DUALPQMOEFullFreezeALORA'
        elif training_args.mode in ['feddualMultipqfullfreezeB_homoAgg_moe']:
            from models.dual_pqlora_freezeB_full_moe.dual_pqloramodel_freezeB_full_moe import Dual_PQMOELorafreezeBModel
            from peft.peft_model import PEFT_TYPE_TO_MODEL_MAPPING
        ...
        elif training_args.mode in ['feddualMultipqfull_homoAgg_moe','feddualMultipqfull_homoAgg_moe2', 'feddualMultipqfull_homoAggOnly_moe','feddualMulti2pqfull_back_homoAgg_moe2',]:
            ...
            lora_config.peft_type = 'DUALPQMOEFullLORA'   
    ...
    if training_args.mode in ['sft_only_B_train', 'fedavg_only_B_train', 'fedours_only_B_train', 'fedours_tv_only_B_train']:
            ...
            if idx in target_layers:
                for n, m in layer.named_modules():
                    if isinstance(m, PQLoraFullLayer):
                        ...
                        m.lora_P['default'] = nn.Parameter(torch.eye((r)))  # Initialized to 1
        ...
        for idx, layer in enumerate(total_layers):
            if idx in target_layers:
                ...
                for n, m in layer.named_modules():
                    if isinstance(m, PQLoraFullLayer):
                        m.use_pq = False
    elif training_args.mode == 'fedMultipqfull' or training_args.mode == 'fedMultipqfull_sft' or training_args.mode == 'fedMultipqfull_sft2':
        from models.pqlora_full.pqloralayer_full import PQLoraFullLayer
        last_layer = len(total_layers) // 4
        target_layers = [last_layer*1 -1,last_layer*2 -1,last_layer*3 -1,last_layer*4 -1]
        for idx, layer in enumerate(total_layers):
    ...
    elif training_args.mode in ['feddualMultipqfullfreeze','feddualMultipqfullfreeze_tv','feddualMultipqfullfreeze_excludemean','feddualMultipqfullfreeze_pqgrad','feddualMultipqfullfreeze_pqfisher','feddualMultipqfullfreeze_pqgrad_homoAgg',
        'feddualMultipqfullfreeze_include','feddualMultipqfullfreeze_tv_include','feddualMultipqfullfreeze_excludemean_include','perada_feddualMultipqfullfreeze','fedsim_feddualMultipqfullfreeze_homoAgg','ditto_feddualMultipqfullfreeze_homoAgg','feddpa_feddualMultipqfullfreeze_homoAgg',
    ...
    elif training_args.mode in ['feddat_Multipqfullfreeze',]:
        ...
        for idx, layer in enumerate(total_layers):
            if idx in target_layers:
                for n, p in layer.named_parameters():
                    if 'lora1_A' in n or 'lora2_A' in n or 'lora3_A' in n:
                        p.requires_grad = False
                    elif 'lora1_B' in n:
                        p.requires_grad = False
                        init_B = torch.empty_like(p)
                        ...
                        dict(layer.named_parameters())[new_n].data = copy.deepcopy(init_B)
                        dict(layer.named_parameters())[new_n].requires_grad = False
                    elif 'lora1_P' in n or 'lora2_P' in n or 'lora3_P' in n or 'lora1_Q' in n or 'lora2_Q' in n or 'lora3_Q' in n:
                        nn.init.zeros_(p)
                for n, m in layer.named_modules():
    ...
    elif training_args.mode in ['feddat_Multi05pqfullfreeze',]:
        from models.triple_pqlora_freeze_full.triple_pqloralayer_freeze_full import TriplePQLoraFullFreezeLayer
        last_layer = len(total_layers) // 2
        target_layers = [last_layer*1 -1,last_layer*2 -1]
        for idx, layer in enumerate(total_layers):
        ...
        for idx, layer in enumerate(total_layers):
            if idx in target_layers:
                ...
                for n, m in layer.named_modules():
                    if isinstance(m, PQMOELoraFullFreezeALayer):
                        m.use_pq = False
    elif training_args.mode in ['feddualMulti2pqfullfreezeA_back_homoAgg_moe2']:
        from models.dual_pqlora_freezeA_full_moe.dual_pqloralayer_freezeA_full_moe import PQMOELoraFullFreezeALayer
        if data_args.is_multimodal:
            if 'llama3.2_3B_vl' in model_args.model_name_or_path:
                if 'front' in training_args.mode:
    ...
    elif training_args.mode in ['feddualMulti2pqfull_back_homoAgg_moe2',]:
        from models.dual_pqlora_full_moe.dual_pqloralayer_full_moe import PQMOELoraFullLayer
        if data_args.is_multimodal:
            if 'llama3.2_3B_vl' in model_args.model_name_or_path:
                if 'front' in training_args.mode:
    ...
    elif training_args.mode in ['feddualMulti2pfullfreeze_back']:
        from models.dual_plora_freeze_full.dual_ploralayer_freeze_full import PLoraFullFreezeLayer
        if data_args.is_multimodal:
            if 'llama3.2_3B_vl' in model_args.model_name_or_path:
                if 'front' in training_args.mode:
            ...
            if idx in target_layers:
                for n, p in layer.named_parameters():
                    if 'lora1_A' in n or 'lora2_A' in n:
                        ...
                        nn.init.zeros_(p)
                for n, m in layer.named_modules():
                    if isinstance(m, PMOELoraFullFreezeLayer):
                        m.freeze_AB = True
            else:
                for n, m in layer.named_modules():
                    if isinstance(m, PMOELoraFullFreezeLayer):
    ...
    elif training_args.mode in ['feddualMulti05pqfullfreeze','feddualMulti05pqfullfreeze_excludemean','feddualMulti05pqfullfreeze_homoAgg', 'feddualMulti05pqfullfreeze_excludemean_homoAgg','feddualMulti05pqfullfreeze_homoAggOnly','perada_feddualMulti05pqfullfreeze','fedsim_feddualMulti05pqfullfreeze_homoAgg','ditto_feddualMulti05pqfullfreeze_homoAgg','feddpa_feddualMulti05pqfullfreeze_homoAgg',]:
        from models.dual_pqlora_freeze_full.dual_pqloralayer_freeze_full import PQLoraFullFreezeLayer
        last_layer = len(total_layers) // 2
        target_layers = [last_layer*1 -1,last_layer*2 -1]
        for idx, layer in enumerate(total_layers):
            if idx in target_layers:
                ...
                for n, m in layer.named_modules():
                    if isinstance(m, PQLoraFullFreezeLayer):
                        m.use_pq = False
    elif training_args.mode in ['feddualMulti05pqfullfreeze_moe', 'feddualMulti05pqfullfreeze_homoAgg_moe','feddualMulti05pqfullfreeze_homoAggOnly_moe','feddualMulti05pqfullfreeze_include_moe','feddualMulti05pqfullfreeze_include_homoAgg_moe','feddualMulti05pqfullfreeze_homoAgg_normalize_moe',]:
        from models.dual_pqlora_freeze_full_moe.dual_pqloralayer_freeze_full_moe import PQMOELoraFullFreezeLayer
        last_layer = len(total_layers) // 2
        target_layers = [last_layer*1 -1,last_layer*2 -1]
        for idx, layer in enumerate(total_layers):
    ...
    elif training_args.mode == 'feddualMultipqfullfreezeA' or training_args.mode == 'feddualMultipqfullfreezeA_tv' or training_args.mode == 'feddualMultipqfullfreezeA_excludemean' or training_args.mode == 'feddualMultipqfullfreezeA2':
    ...
    elif training_args.mode == 'feddualMulti2pqfullfreezeA_back2':
        from models.dual_pqlora_freezeA_full.dual_pqloralayer_freezeA_full import PQLoraFullFreezeALayer
        # last_layer = len(total_layers) // 4
        # target_layers = [last_layer*1 -1,last_layer*2 -1,last_layer*3 -1,last_layer*4 -1]
        if data_args.is_multimodal:
    ...
    elif training_args.mode == 'feddualMultipqfull' or training_args.mode =='feddualMultipqfull2':
        ...
        for idx, layer in enumerate(total_layers):
            if idx in target_layers:
                ...
                for n, m in layer.named_modules():
                    if isinstance(m, PQLoraFullLayer):
                        m.use_pq = False
    elif training_args.mode == 'feddualMulti2pqfull_back2':
        from models.dual_pqlora_full.dual_pqloralayer_full import PQLoraFullLayer
        # last_layer = len(total_layers) // 4
        # target_layers = [last_layer*1 -1,last_layer*2 -1,last_layer*3 -1,last_layer*4 -1]
        if data_args.is_multimodal:
    ...
    if training_args.load_pretrained_random or training_args.load_pretrained_orthnorm or training_args.load_pretrained_pca:
        if training_args.mode in ['fedMultipqfullfreeze_sft', 'fedMultipqfullfreeze', 'fedMultipqfullfreeze_tv', 'fedMultipqfullfreeze_ours', 'fedMultipqfullfreezeA', 'fedMultipqfreezeA', 'fedMultipqfreezeA_sft', 'feddualMultipq_freezeA_trainB_weightnormP',
                                  'fedMultipqfullfreezeA_sft','fedMultipqfullfreezeB_sft','fedMultipqfull_sft','fedMultipqfull_sft2',
            ...
            if training_args.load_pretrained_orthnorm:
                ...
                if 'llama3.2_1B_vl' in model_args.model_name_or_path:
                    ...
                    state_dict = torch.load('llava_3b_blockwise_cca_init.pth', map_location='cpu')
            ...
            for k, v in state_dict.items():
                ...
                if training_args.mode in ['feddualMultipqfull_homoAgg_moe', 'feddualMultipqfull_homoAggOnly_moe', 'feddualMultipqfullfreezeA_homoAgg_moe','feddualMultipqfullfreezeA','feddualMultipqfull','feddualMulti2pqfullfreezeA_back_homoAgg_moe','feddualMulti2pqfull_back_homoAgg_moe',]:
                    if 'lora_P' in k:
                        ...
                        nn.init.kaiming_uniform_(v, a=math.sqrt(5))
                if training_args.randomize_B and 'lora_B' in k and v.sum() != 0:
                    nn.init.kaiming_uniform_(v, a=math.sqrt(5))
                ...
                if 'feddat' in training_args.mode:
                    new_k3 = k.replace('lora', 'lora3')
                    new_state_dict[new_k3] = v
...
def get_keys_to_del(training_args, new_global_state_dict, data_args):
    ...
    if training_args.mode in ['fedours', 'fedours_tv', 'fedours_excludemean', 'fedours_include', 'fedours_tv_include', 'fedours_excludemean_include', 'fedours_excludemean_hetero', 'fedours_pqgrad_moe','fedours_moe_smooth_linearC', 'fedours_moe_smooth_linear', 'fedours_moe_smooth_ema',
                              ...
                              'feddualMultipqfullfreezeA_homoAgg_moe','feddualMultipqfullfreezeB_homoAgg_moe','feddualMultipqfull_homoAgg_moe','feddualMultipqfull_homoAgg_moe2','feddualMultipqfull_homoAggOnly_moe','feddualMultipqfullfreezeA_homoAgg_moe2','feddualMulti2pqfullfreezeA_back_homoAgg_moe2','feddualMulti2pqfull_back_homoAgg_moe2',
                              'feddualMultipqfullfreeze32_homoAgg_moe','feddualMultipqfullfreeze64_homoAgg_moe','feddualMultipqfullfreeze256_homoAgg_moe','feddualMultipqfullfreeze512_homoAgg_moe','feddualMultipqfullfreeze1024_homoAgg_moe',
                              'feddualMultipqfullfreeze_excludemean_homoAgg_moe','feddualMultipqfullfreeze_pqgrad_homoAgg','feddualMultipqfullfreeze_pqgrad_homoAgg_moe',
                              ]:
    ...
    elif training_args.mode in ['feddat','feddat_Multipqfullfreeze', 'feddat_Multi05pqfullfreeze',]:
        for k in new_global_state_dict.keys():
            if 'lora2' in k or 'lora3' in k:
               keys_to_del.append(k)
        ...
        for k in new_global_state_dict.keys():
            if 'layers.' in k and int(k.split('.')[layer_index]) in layers_to_del or ('lora_P' not in k and 'lora_Q' not in k):
                keys_to_del.append(k)
    elif training_args.mode in ['fedMulti05pqfullfreeze','fedMulti05pqfullfreeze_sft']:
        layer_num = []
        for k in new_global_state_dict.keys():
            if 'layers.' in k:
                layer_num.append(int(k.split('.')[layer_index]))
        ...
        for k in new_global_state_dict.keys():
            if 'layers.' in k and int(k.split('.')[layer_index]) in layers_to_del or ('lora1_P' not in k and 'lora1_Q' not in k):
                keys_to_del.append(k)
                                ...
                                'feddualMultipqfullfreezeA','feddualMultipqfullfreezeA_tv','feddualMultipqfullfreezeA_excludemean','feddualMultipqfull','feddualMultipqfull2','feddualMultipqfullfreezeA2',
        ...
        for k in new_global_state_dict.keys():
            if 'layers.' in k and int(k.split('.')[layer_index]) in layers_to_del or ('lora1_P' not in k and 'lora1_Q' not in k):
                keys_to_del.append(k)
                                ...
                                'feddualMulti2pqfullfreeze_back_moe','feddualMulti2pqfullfreezeA_back2','feddualMulti2pqfull_back2',]:
        layer_num = []
        for k in new_global_state_dict.keys():
            if 'layers.' in k:
                layer_num.append(int(k.split('.')[layer_index]))
